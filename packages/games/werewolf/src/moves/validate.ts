...
import type { WerewolfMasterState } from '../types/index.js';
import { RoleRegistry } from '../roles/registry.js';
import { validateRoleCounts } from '../engine/role-assignment.js';

export function validateWerewolfMove(
  ...
  const isHost = player.isHost || (state.hostMode && player.id === state.hostPlayerId);
  ...
  switch (move.type) {
    case 'UPDATE_ROLE_CONFIG': {
      ...
      if (!isHost) {
        return { valid: false, reason: 'Only the Host can configure roles.' };
      }
      return { valid: true };
    }
    ...
    case 'START_ROLE_SETUP': {
      ...
      if (!isHost) {
        return { valid: false, reason: 'Only the Host can start role setup.' };
      }
      const playingPlayers = state.players.filter((p) => p.canPlay !== false && !p.isHost);
      const val = validateRoleCounts(state.roleCounts, playingPlayers.length);
      if (!val.valid) {
    ...
    case 'SELECT_ROLE': {
      ...
      const roleDef = RoleRegistry.getInstance().get(roleId);
    ...
    case 'CONFIRM_ROLE': {
      ...
      if (player.isHost || player.canPlay === false) {
        return { valid: false, reason: 'The Host does not confirm a player role.' };
      }
      if (!player.roleId) {
    ...
    case 'START_NIGHT_1': {
      ...
      if (!isHost) {
        return { valid: false, reason: 'Only the Host can start Night 1.' };
      }
      return { valid: true };
    }
    ...
    case 'CONFIRM_START': {
      ...
      if (state.hostMode && state.hostPlayerId && move.playerId !== state.hostPlayerId) {
        return { valid: false, reason: 'Only the Host can start the game.' };
      }
      ...
      if (state.phase === 'ROLE_SELECTION' && unselected.length > 0) {
        return {
          valid: false,
          reason: `Waiting for ${unselected.length} player(s) to select a role.`,
        };
      }
      return { valid: true };
    }
    ...
    case 'DAY_VOTE': {
      ...
      if (targetId !== 'SKIP') {
        ...
        if (!target || !target.isAlive) {
          return { valid: false, reason: 'Vote target must be an alive player.' };
        }
      }
      return { valid: true };
    }
